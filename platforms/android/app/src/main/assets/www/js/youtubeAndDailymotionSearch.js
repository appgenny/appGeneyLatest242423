  ...
  $.ajax(t).done(function (e) {
    ...
    console.log(o),
      $.each(o, function (e, o) {
        ...
        (t +=
          '<div style="margin-bottom:10px;" class="col s12 single_post_view">'),
          (t += '<div class="entry video_list_div">'),
          (t +=
            '<a vedioId="' +
            i +
            '" platform="youtube" style="color:#000 !important" onclick="videoModel(this)" >'),
          (t +=
            '<img class="lazyloadingImage" src="' +
            s +
            '" alt="" style="padding: 4px;border: 1px solid #c4c4c4;width: 100%;max-height: 235px;">'),
          (t += "</a>"),
          (t += '<div class="desc">'),
          (t +=
            '<a vedioId="' +
            i +
            '" platform="youtube" style="color:#000 !important" onclick="videoModel(this)" ><h4 style="font-size: 26px;margin-left: 2px;font-family: timenewroman;font-weight: bold;padding-top: 8px;">' +
            a +
            "</h4></a>"),
          (t += "</div>"),
          (t += "</div>");
...
function getyoutubeVideoBySearch(e, o, t) {
  var youtubeKey = $.trim(JSON.parse(localStorage.getItem("item"))[6]);
  var i = encodeURI(
...
function getDailymotionBySearch(e, o) {
  var t = encodeURI(
    "https://api.dailymotion.com/videos?search=" +
      ...
      "&limit=" +
      o +
      "&fields=id,title,channel,owner,thumbnail_large_url"
  );
  $.ajax({
    type: "GET",
    url: t,
    success: function (e) {
      ...
      $.each(o, function (e, o) {
        ...
        var s = o.thumbnail_large_url;
        o.channel;
        (t +=
          '<div style="margin-bottom:10px;" class="col s12 single_post_view">'),
          (t += '<div class="entry dailymotion video_list_div">'),
          (t +=
            '<a vedioId="' +
            i +
            '" platform="dailymotion" style="color:#000 !important" onclick="videoModel(this)" ><h4 style="font-size: 26px;margin-left: 2px;font-family: timenewroman;font-weight: bold;padding-top: 8px;">'),
          (t +=
            '<img class="lazyloadingImage" src="' +
            s +
            '" alt="" style="padding: 4px;border: 1px solid #c4c4c4;width: 100%;max-height: 235px;">'),
...
function videoModel(e) {
  ...
  if ("admobRunning" == a && s > 0) {
    ...
    if ((d = localStorage.getItem("counterAds")) == (s = n[2])) {
      console.log("admobLimit" + s),
        console.log("counterAds" + d),
        showIndustrialAdForYoutubeVIdeoPlayer();
      localStorage.setItem("runVideoId", o),
        localStorage.setItem("runVideoplatform", t),
...
function runVideoPlayer(e, o) {
  "dailymotion" == o &&
    ...
    (videourl =
      "https://www.dailymotion.com/embed/video/" + e + "?queue-enable=false"),
    $("#videoplayer").attr("src", videourl)),
    "youtube" == o &&
      YoutubeVideoPlayer.openVideo(e, function (e) {
